...
> `VISION.md`. Where `VISION.md` renders what Atticus *is* and *is for*, this
> document is the blueprint one layer down — how that vision is built. It is
> descriptive, not a plan: present-tense, no phases, no to-do list, no roadmap.
> `VISION.md` is the oracle. Where the two disagree, *this* document is wrong
> and is brought into line — never the reverse. The two are paired core
> documents and evolve together.
...
collaborator, several jobs, brain-backed, speaking through Hermes. This document
fixes *how that shape is realized*: the layers Atticus decomposes into, the file
formats and directory structure, the page taxonomy and frontmatter schema, the
seam to the Hermes body, the substrate stores, and the sync architecture.

It describes the designed system in the present tense — the way a blueprint
draws a house that is not yet built. A reader who opens the repository today
will find construction in progress; this document describes the structure that
...
Atticus decomposes into **four layers** (`VISION.md` §3.1). Three of them are
pure markdown — portable by nature, a movable directory. The fourth is tool
bindings and belongs to the body.

| Layer | What it is | Form | Owned by |
|---|---|---|---|
| **Character** | Who Atticus *is* — voice, posture, the inviolable spine | Markdown (`SOUL.md`, `STYLE.md`) | Atticus |
| **Skills** | How Atticus *works* on a job | Markdown procedures (`skills/`) | Atticus |
| **Capabilities** | What Atticus *can do* — fetch, read mail, send | Tool bindings | Hermes |
| **Brain** | What Atticus *knows* — the knowledge graph | Markdown vault | Atticus |

**Atticus is the brain, the character, and the skills** — and nothing else. As
far as possible it is *pure markdown*: a directory of files, durable and
portable. **The harness is the body and the voice** — it owns every mechanic
(the live LLM loop, scheduling, the terminal UI, messaging gateways, voice,
OAuth, proactive delivery). The harness loads Atticus's brain, character, and
skills and in doing so *becomes* Atticus. Bryan talks to "Atticus"; the
process running is the harness (`VISION.md` §3). **Hermes is the reference
harness** and the most fully capable one; Claude Code and other harnesses are
supported to the extent that their capability sets (§5) overlap with what each
skill needs — see `docs/harnesses/` for per-harness adapters.

Atticus's substrate is **two harness-independent stores**, neither tied to any
particular harness:

1. **The markdown brain** — Bryan's work and knowledge (§2). The model of
   Bryan himself lives in `BRYAN.md` at the root of the brain (§7); a
   sidecar `BRYAN-OBSERVATIONS.md` exists as staging for the
...
Atticus.** Portability is a consideration, not a constraint. The markdown
...
infrastructure makes Atticus better, it binds; the honest asymmetry that
...
The brain is what Atticus knows. It is a tree of markdown files — inert,
...
├── SOUL.md              # character — who Atticus is (§3.1)
├── STYLE.md             # scientific-writing voice (§3.2)
├── BRYAN.md             # declared model of Bryan — the user (§7.2)
├── RESEARCH.md          # state of the research program (§2.6)
├── skills/              # skill procedures + RESOLVER.md (§4)
├── papers/              # ─┐
├── methods/             #  │
...
Atticus is **literature-native**: papers, methods, concepts, and hypotheses are
first-class; people are real but de-emphasized (`VISION.md` §2.4, and the
research framing throughout §2).
...
| `hypothesis` | A testable claim Atticus tracks, with evidence pro and con |
...
remain plain markdown that any tool can read.
...
Atticus owns no code, so the score is not computed mechanically. It is
...
`BRYAN.md`, §7) and not a model of Atticus (that is `SOUL.md`, §3). Skills
...
a cache. Transplanting Atticus to a new harness means copying the directory
and letting the new harness rebuild its own index — qmd, ripgrep, or
whatever the new harness provides for `brain-search`.

---

## 3. The character — who Atticus is

*Implements `VISION.md` §2.5, §3.1 layer 1.*

The character is who Atticus is regardless of task. It is pure prose, portable
...
- **A rich body of internalized dispositions** — the cognitive patterns,
  ...
  every response without being rules. They are what make Atticus brilliant.

`SOUL.md` §3 is the **authoritative set of cognitive patterns** — the thinking
instincts of a scientific collaborator at the immunology × AI interface
(mechanism hunger, the discriminating experiment, confound scan, the novelty
premium, and the rest). `VISION.md` §2.5 reproduces them verbatim; if the set
evolves, both move together.

Hermes loads `SOUL.md` as the character layer at the start of every session —
it is in context for every response. Dispositions are *internalized, never
narrated*: Atticus does not announce which instinct it is applying.

### 3.2 STYLE.md

`STYLE.md` is the scientific-writing companion to `SOUL.md` — the character
applied to the page. It codifies Bryan's learned voice, the **single-reader
model** (every application written for one smart scientist whose expertise sits
adjacent to the proposal), and the discipline that keeps finished prose free of
machine-writing tells (`VISION.md` §2.2). Where `SOUL.md` governs how Atticus
*thinks*, `STYLE.md` governs how it *writes*. Grant-writing skills (§4.2) load
it; other skills do not.
...
## 4. Skills — how Atticus works

*Implements `VISION.md` §2.1–2.3, §3.1 layer 2, §5.*

### 4.1 Skills as markdown procedures

A skill is a fat markdown file under `skills/` — a procedure for one
recognizable job. Skills are tool-agnostic prose: they prefer to name a
...
Atticus's three jobs (`VISION.md` §2) are **not personas and not modes**. They
...
Hermes executes the fetch. New literature sources are new markdown, not new
software.
...
- **A relevance bar drawn from the user model.** What matters *to Bryan
  specifically* comes from `BRYAN.md` (§7), not a global heuristic.
...
- **An auditable ignore-report.** A regular, explicit "here is what I decided
  you could ignore," so the filter is legible and Bryan can correct it. A
  black-box filter cannot earn trust.

The bar for this layer is trust: it succeeds only if Bryan can stop compulsively
checking his own inbox.

---

## 5. Capabilities — what Atticus can do

*Implements `VISION.md` §3.1 layer 3, §3.2.*

### 5.1 The capability tiers

Capabilities are tool bindings — *not* markdown — and Hermes provides them. They
fall in three tiers (`VISION.md` §3.2):
...
Atticus meaningfully better. Build the best possible Atticus first; keep the
coupling loose where loose coupling costs nothing.

---

## 6. The Atticus–harness seam

*Implements `VISION.md` §3, §3.2.*

### 6.1 The split

A harness loads Atticus and becomes it. The reference harness is **Hermes** —
a fat harness that ships its own scheduler, skill system, cross-session
memory, MCP-client support, and a messaging gateway. Other harnesses
(Claude Code, future) have different capability sets; the Atticus side of the
seam is the same. The split is ratified (`VISION.md` §3):

- **Atticus owns**: the brain, the character, the skills — pure markdown.
...
Per-harness adapter docs in `docs/harnesses/` map each Atticus capability
...
`user-model-query` is the same on every harness now (§7); it always returns
`{declared: BRYAN.md}`.

### 6.2 How a harness becomes Atticus

The harness points at the brain directory. It loads `SOUL.md`, `STYLE.md`,
`BRYAN.md`, and `AGENTS.md` as the always-on layer, discovers `skills/`,
reads `skills/RESOLVER.md` for routing, and builds (or borrows) a search
index over the page tree. With those loaded, the running harness process *is*
Atticus. The **Atticus side of the seam contract** is exactly this set of
file conventions — the character files and `RESEARCH.md` at the root, the
`skills/` tree with `RESOLVER.md`, the page directories with their
frontmatter schema. Any harness that honors those conventions can host
Atticus.

### 6.3 Atticus generates, the harness delivers

The live seam runs one direction: **Atticus generates, the harness delivers**.
The morning brief is composed by an Atticus skill against the brain; the
harness is what pushes it to Telegram (or whatever channel the harness
provides) at 7am. A deadline nudge is decided by the attention contract
(§4.5); the harness is the channel it rides. Atticus never owns a transport.

### 6.4 What this means for the inherited stack

Atticus owns no runtime mechanics. There is no Atticus-side scheduler, no
Atticus-side server transport, no autopilot daemon, and no database engine
that Atticus *requires* to exist. Where the harness already provides a
mechanism, Atticus does not carry its own. The search index (§2.7) is the one
piece of heavy machinery near the brain, and it is harness-owned and
disposable — a cache over the markdown, not a component of Atticus.

---

## 7. The model of Bryan — `BRYAN.md`

*Implements `VISION.md` §3.3, and the theory-of-Bryan in §2.1 and §5.*

### 7.1 The user model is `BRYAN.md`

A *Power of Two* partnership needs a theory of Bryan — his taste, intellectual
style, recurring blind spots, what excites or bores him. That theory lives in
one markdown file at the root of the brain (alongside `SOUL.md`), always
loaded, harness-independent by nature:

- **`BRYAN.md`** holds what Bryan says about himself — research priors,
  working style, named blind spots, how he wants to be engaged, what's in
  and out of scope. Bryan owns it; Atticus may propose edits in conversation
  but does not auto-write to it. Bryan refreshes it by hand, typically in
...
A sibling file, **`BRYAN-OBSERVATIONS.md`**, exists as a *staging surface*
for the `user-model-reflect` skill (§7.3). It is:

- **Not always loaded.** Atticus does not consult it in conversation.
- **Not part of `user-model-query`'s return shape** (§7.2).
- A working notes file Bryan reads when he wants to refresh `BRYAN.md`.

The reason `BRYAN.md` is the only consulted layer is parity: every harness
gets the same user model, because the user model is just one file. The
earlier design used a third-party inferred layer (Honcho, part of Hermes),
which made harness parity structurally impossible; that decision is
reversed in `docs/decisions/honcho-removal.md`, which supersedes
...
- **`SOUL.md` models Atticus** — who Atticus is being (§3.1).
- **`STYLE.md` models the page** — how Atticus writes finished prose (§3.2).

### 7.2 How Atticus uses the user-model layer
...
The thought partner consults the layer to know how to engage Bryan and where
his blind spots lie; the attention contract (§4.5) consults it for the
relevance bar. The old static, hand-maintained `USER.md` is superseded by
`BRYAN.md`.

### 7.3 Enrichment — manual, on demand

`BRYAN.md` is always loaded and current by definition: Bryan owns it and
edits it directly, and that is the rhythm by which the user model evolves.
A skill — **`user-model-reflect`** — exists to help Bryan see what would be
worth adding. It reads recent session transcripts via
...
`BRYAN.md` and never duplicates content already there. If the harness does
not expose conversational history, the skill writes a no-op stub for the
date and surfaces the missing capability cleanly.

**No schedule is wired today.** The skill is invokable on demand
...
Promotion stays with Bryan: he reads `BRYAN-OBSERVATIONS.md` when he wants
to refresh the spine, edits the relevant section of `BRYAN.md` to absorb
what is durable, and prunes the promoted entry from the sidecar. There is
no skill that promotes for him — the spine of the user model stays under
Bryan's hand, the same way `SOUL.md` does.
...
critiques as PDF or DOCX, other documents Bryan feeds in. These are the ground
truth and must never be lost. They live in an **S3-compatible object store
(Cloudflare R2)**, **write-once and content-addressed**, with **no file-size
limits**. Every binary source lands in R2; nothing binary lands in git.
...
1. Bryan drops a PDF or DOCX into `_drop/` (on a laptop, or via a messaging
   attachment that the harness routes to the same folder as a capture
   fallback).
2. Syncthing (§9.2) carries it to the harness host. **The host has no
   filesystem watcher** — Hermes does not ship one, and harness-side
   filesystem events are not part of the seam (§6). Instead, a cron job
   polls `_drop/` on an interval (the polling cadence is a harness-side
   configuration choice — see `SETUP.md`); when it finds a new file, it
   triggers the ingest skill.
3. Atticus ingests it — extracts text, chunks it, and files brain pages with
   citations.
...
`watch-path` capability and skip the poll; today no supported harness does.

Non-markdown formats are *sources*, never brain pages: Atticus reads a DOCX,
...
Bryan edits a grant draft in Obsidian; the file changes on disk; the harness
picks it up on its next poll (§8.4 — same poll-not-watch substitution). The
end-to-end edit-detection latency is the same as the `_drop/` polling cadence.
Atticus writes a page and it appears in Obsidian. There is no "did Bryan edit
a document" problem to solve — editing a document and editing the brain are
one act.
...
- **The Hermes host holds the canonical vault** and stays headless and
  Obsidian-free. Atticus reads and writes the markdown files directly — plain
  file IO, no Obsidian process on the host.
- **Syncthing** propagates the vault between the host and Bryan's laptop(s) —
  continuous, peer-to-peer, over the existing Tailscale mesh, no cloud
  ...
  rest of `.obsidian/` may sync so Bryan's setup matches across laptops.
...
- **Host-side git, pushed to a private GitHub repo.** The host auto-commits and
  ...
  Bryan changed), version history, an off-site backup, and the iOS bridge.

### 9.3 The iOS bridge

Syncthing on iOS is weak, so the phone reaches the brain through GitHub instead.
The GitHub iOS app renders formatted markdown and commit diffs well. Hermes
sends Telegram messages with commit-pinned GitHub permalinks to notable
just-committed files, so Bryan taps straight to the thing — and can review
exactly what Atticus changed — rather than scrolling a directory.

### 9.4 The conflict story

The phone is a **consumption and review surface, not a generation surface**
(`VISION.md` §4.1). Bryan does not draft significant text on the phone; his
quick feedback flows through Telegram, not by editing markdown. The phone is
therefore effectively read-only on the vault.

That leaves only two writers: Atticus (on the host) and Bryan (via Obsidian on a
laptop → Syncthing → host). They mostly touch different files. The one case that
matters — a live grant draft — is handled by a behavioral rule, not by sync
...
GitHub for the iOS bridge, R2 for the archive.
...
Atticus is reachable through three surfaces, each matched to the job it is good
at. The surfaces are Hermes capabilities (§5); Atticus generates, Hermes
delivers (§6.3).
...
- **Telegram** — asynchronous, lightweight. Excellent for *capture* (a voice
  memo of an idea, a "remind me about X") and *notification* (the morning brief,
  a deadline nudge). The ambient, always-in-pocket channel.
...
are out of scope, decided, not deferred: **personal-life content** and
**lab operations** (ordering, equipment logistics, managing the 1:1 cadence).
Each is better served by a separate, independent setup. (Revised 2026-07-30:
the exclusion narrowed from "lab-state management" to operations churn; lab
capability knowledge — org structure, member expertise, project status — is in
scope as research-program knowledge. Design record:
