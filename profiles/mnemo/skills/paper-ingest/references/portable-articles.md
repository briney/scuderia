...
Archive-only plans skip this section. Page plans require `candidate-import --submission ...`, inspection of `page-candidate/page-candidate.md` and its diff, then separately authorized `apply --authorize-page-apply`. Inspect the generated qualification register and total page size as well as the scientific prose. New page registers use `portable-page-qualification-register-v4`: readable scoped qualifications and archive evidence pointers, with a compact machine binding. The register addresses compiled final evidence, with no dependency on old page snapshots or review packets. Historical v1/v2/v3 registers and completion receipts retain their original rendering and verification contract; do not rewrite completed receipts.
