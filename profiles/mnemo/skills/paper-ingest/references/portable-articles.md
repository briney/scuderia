...
Load for an explicit article-package archive/restore or existing-paper re-enrichment request. This is one shared Python library/CLI (`scripts/portable_articles.py`, `scripts/reenrich.py`), not a second acquisition pipeline or an unattended campaign. New ingestion and refresh share `article_enrichment`; source acquisition/extraction uses the current readiness policy. The operator supplies retrieval, scientific review and page reconciliation through explicit contracts; pending operator work is not completion.
...
Use the instance's accepted PDF Python interpreter, rclone remote/bucket/prefix and trusted code roots. Set `PDF_ENRICHMENT_METHOD`, `REENRICH_ENRICHMENT_ROOT` and `REENRICH_INTEGRATION_ROOT` to the same resolved skill `scripts/` directory (`runtime.md`). Never execute code or choose credentials from paths embedded in an archive. Set `PYTHONDONTWRITEBYTECODE=1`; use an explicit nonsymlink scratch directory only for disposable tests/caches. Active jobs require durable operational storage until finalization; permanent retention is limited to final products. No installation, scheduled run, paid call, archive write or page mutation is authorized merely by loading this reference.
...
Existing frozen qualified evidence uses `--job`, `--review` and `--export` together. Optional `--page` and repeated `--legacy-pdf` are historical input options, not the permanent retention policy. `--document-bindings` supplies exact document/acquisition associations when required. Never use `--fixture-root` on a production package.

Before publishing a manually assembled source representation, compile its final products:

    <py> -B <scripts>/final_products.py compact --manifest <temporary-archive/manifest.json> --output <new-final-dir>
    <py> -B <scripts>/final_products.py verify --manifest <new-final-dir/manifest.json>

`compact` preserves evidence; it does not attest that a page is complete or replace old page receipts. Initial ingestion uses the verified `ingest` finalizer in `qualified-enrichment.md`; refresh `publish` finalizes automatically. Publish the final manifest, never the temporary whole-run representation.
...
Archive and consumer retention are not model context. Original sources, final scientific results, their provenance and substantive qualifications remain addressable. Temporary raw review/request history is not a permanent dependency. The shared production/diagnostic prompt projects selected-element and source-document findings using recorded document/hash/element and directory associations, not shared labels or page numbers. Actual unscoped warnings and attribution remain explicit; empty hash-only entries are not sent. Identical decoded projections are represented once without merging different targets, sources or attribution. Surrounding native page text and source limitations remain context, not verified element associations. New results do not resolve inherited findings automatically. There is no text cap or model-based pruning; an oversized relevant prompt still fails the official context gate.

## Optional page reconciliation

Archive-only plans skip this section. Page plans require `candidate-import --submission ...`, inspection of `page-candidate/page-candidate.md` and its diff, then separately authorized `apply --authorize-page-apply`. Inspect the scientific prose and total page size. New plans set `page_register_location: archive`; their `portable-page-qualification-register-v4` metadata lives in the sealed candidate record and then the archived manifest and JSON completion receipt. No register, visible or hidden, is inserted into paper Markdown. Keep concise claim-relevant caveats in the prose. The register addresses compiled final evidence without depending on old page snapshots or review packets. Historical embedded registers and completion receipts keep their original verification contract; do not rewrite completed receipts.
...
For an attributed no-change review, set `reconciliation_outcome: reviewed-no-scientific-text-change` and keep each reviewed body exactly unchanged. Do not invent scientific edits to pass the gate. The code records source/element/target qualifications, attributed correction proposals, export hashes and a publication-receipt locator in archive metadata. A verified old embedded register is removed during candidate generation; figure ownership markers remain unchanged. Recorded source/version/element and target relationships scope inherited warnings; genuinely unscoped warnings remain explicit. Identical qualifications are represented once. Previously cited targets and their qualifications survive subsequent selective refreshes. Before migrating an embedded register or inheriting an archived register, the reader verifies its archived page snapshot, exports, source locators, review bindings and inherited evidence. Missing evidence or edits inside the old register hold replacement; restore the required archive objects before continuing. A missing publication receipt still means publication is pending. Generic caution alone cannot replace scoped findings. Source/graph links, human annotations and unrelated sections remain protected; a changed live page hash holds application. Pause noncooperating editors during application.
...
Omit `--page` only for archive-only plans. For finalized packages the verifier checks source/product hashes, the complete final inventory, compiled evidence/register pointers (from archive metadata for new plans) and the exact applied page/companion receipt. It does not replay requests or reconstruct review packets. Historical receipts use their original verifier. It works after original run roots are gone. Publication verification is historical; for fresh remote availability, restore using trusted transport/pins and then verify. Subsequent manual page edits invalidate exact-snapshot completion and require a new reviewed revision, not overwriting an old receipt.
...
For existing packages, build a separate compact directory, compare original hashes and scientific products/qualifications, and migrate page/receipt pointers before deleting old files. An unfinished enrichment is not disposable merely because its source extraction completed. No automatic historical remote-object deletion is provided.
