...
brief: same brain, same attention contract, but the output is what your human should
...
- Every meeting today is loaded with context: attendee `people` pages, the
  thread connecting them to your human, the open items between them.
...
1. **Calendar lookahead.** For each `meeting` page dated today, build a prep
   ...
   they are, recent threads with your human, open items between them. End each card
   with the prep itself: what your human should know walking in, what decision the
...
Cite facts inline so your human can trace them, and name coverage gaps — an attendee
