# VISION.md — the mnemo archetype

> **Status: v1.4 — vision settled; re-homed to the profile.** This is the
> big-picture vision for what a **mnemo mind** *is for* — the research-brain
> archetype, scuderia's flagship profile. Its major questions have been
> worked through and decided; §8 is the decision log. It is deliberately
> implementation-light — the blueprint is `DESIGN.md` in this directory,
> which carries the *how*. Where the two disagree, *this* document is the
> intent and `DESIGN.md` is brought into line.
>
> v1.4 note (2026-08-28): the north-star documents split into two layers.
> The **platform** vision — the stable, the driver, why several agents
> rather than one — lives at `docs/north-star/VISION.md` in the scuderia
> repo. This document is the **mnemo archetype** vision and lives with the
> profile it describes; nothing in it claims every scuderia agent is
> mind-shaped. Platform vocabulary (ratified): the general category is the
> **agent**; **mind** is mnemo's self-description — a mnemo agent is a mind
> because a thinker is what it is. An ergon instance is a *doer*, and its
> persistent state is a capability library, not a knowledge graph.
> Inter-agent collaboration (a doer working with a mind) is governed by
...
9. **Platform vocabulary.** ✓ Resolved (2026-08-24): the general
...
10. **Document home.** ✓ Resolved (2026-08-28): the north-star pair split
    into platform and archetype layers. The platform vision and blueprint
    (`docs/north-star/`) describe the stable; this document and its
    `DESIGN.md` companion moved into `profiles/mnemo/` as the mnemo
    archetype's own north-star. Content carried over unchanged in
    substance; only scope statements were rewritten.
