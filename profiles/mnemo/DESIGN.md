# DESIGN.md — the mnemo archetype

> **The mnemo implementation blueprint.** This is the technical companion to
> the mnemo `VISION.md` in this directory. Where `VISION.md` renders what a
> mnemo mind *is* and *is for*, this document is the blueprint one layer
> down — how that vision is built. It is descriptive, not a plan:
> present-tense, no phases, no to-do list, no roadmap. `VISION.md` is the
> oracle. Where the two disagree, *this* document is wrong and is brought
> into line — never the reverse. The two are paired core documents and
> evolve together.
>
> v1.4 note (2026-08-28): re-homed to the profile when the north-star pair
> split. **Platform-level machinery** — the instance contract, profile
> anatomy, the harness seam in its general form, the agora, the feed — is
> specified in `docs/north-star/DESIGN.md` at the repo root; this document
> covers the mnemo archetype: its page kinds, its knowledge graph, its
> character files, its attention contract. Where both documents touch the
> same machinery (the seam, §6), the platform document is the general
> contract and this one is the mnemo binding of it.
