...
description: Use when starting feature work that needs isolation from the current workspace or before executing implementation plans — detect existing isolation, prefer native worktree tools, fall back to git worktree.
triggers:
  - "isolated workspace"
  - "worktree"
...
© 2025 Jesse Vincent), vendored 2026-08-24. Nearly verbatim — already
harness-agnostic. Hermes note: the native worktree tool is `hermes -w`
(worktree mode at session start); mid-session, use the git fallback (1b).*
...
have a way to create a worktree? (Under Hermes: a session started with
`hermes -w`. Mid-session there is no native tool — use 1b.) If you do, use it
and skip to Step 2.
