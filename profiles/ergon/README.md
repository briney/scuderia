...
A mnemo mind's persistent state is a knowledge graph; an ergon mind's
persistent state is a **capability library** — its skills, its run log, and
its craft knowledge (tool quirks, environment facts, failure modes). It
holds **no domain data**: when it needs to know something, it asks a
knowledge mind, fresh, every time. Its products are artifacts — verified,
provenance-carrying, deposited in the shared store.
...
v1 scope: ergon minds are **reactive only** — they answer queries and
